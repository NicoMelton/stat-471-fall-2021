...
# how to split into train and testing based on percentage
...
# or 
readmission_train %>% filter(age_group == "0-19") %>% count(adm_source) %>%
  rename(Frequency=n) %>%
  kable(format = "latex", 
        row.names = NA,
        booktabs = TRUE,
        ...
        caption = "Frequencies of admission source for patients in the age of 0-19") %>%
...
# fill in the code here
readmission_train %>%
  ggplot(aes(x = time_in_hospital, y = as.factor(readmitted))) +
  geom_boxplot() +
  theme_bw()
...
set.seed(3)
# fill in the code here
lasso_fit = cv.glmnet(readmitted ~ ., 
                      family = "binomial", 
                      alpha  = 1,
                      nfolds = 10,
                      readmission_train)
